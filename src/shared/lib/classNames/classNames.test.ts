...
describe('classNames', () => {
  ...
  test('with additional class and mods', () => {
    ...
    expect(classNames('someClass', { hover: true, light: true }, ['additional1', 'additional2'])).toBe(expected);
  });
  test('with additional class and mods false', () => {
    const expected = 'someClass additional1 additional2 hover light';
    expect(classNames('someClass', { hover: true, rounded: false, light: true }, ['additional1', 'additional2'])).toBe(expected);
  });
  test('with additional class and mods undefined', () => {
    const expected = 'someClass additional1 additional2 hover';
    expect(classNames('someClass', { hover: true, rounded: false, light: undefined }, ['additional1', 'additional2'])).toBe(expected);
  });
});
